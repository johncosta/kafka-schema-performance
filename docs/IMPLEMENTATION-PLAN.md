...
| **Distributed-style proxy** | `tests/test_distributed_performance.py` — `@pytest.mark.distributed`: **large** (and **medium** wire) payloads where JSON is larger on the wire than Avro/Protobuf; **S1** compressed size ordering; **S3/S4** batch `effective_records_per_s` on **large** where binary codecs exceed JSON by a margin (in-memory producer/consumer proxies, not a real Kafka cluster). |
