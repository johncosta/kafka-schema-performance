...
| **Kafka E2E** | `docker/docker-compose.kafka.yml` (Redpanda, Kafka protocol) + `make test-kafka` or `KSP_KAFKA_BOOTSTRAP`. `tests/integration/` — `@pytest.mark.kafka`: synchronous produce + consume via `kafka-python-ng`; metrics as **`kafka_e2e`** in `report.json` (`benchmark.integration.kafka_e2e` + `attach_kafka_e2e_to_report`). **Summary HTML** renders a Kafka section when that key exists. Testcontainers only if **`KSP_USE_TESTCONTAINERS=1`**. |
